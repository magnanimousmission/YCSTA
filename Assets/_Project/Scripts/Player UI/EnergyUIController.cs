...
public class EnergyUIController : MonoBehaviour
{
    ...
    public void Bind(PlayerCore playerCore)
    {
        ...
        var energy = _playerCore.GetPlayerData().energy;
        _displayValue = energy;
        _targetValue = energy;

        _text.text = $"Stamina: {Mathf.RoundToInt(energy)}";

        _playerCore.OnEnergyChanged += SetTarget;
    }
    ...
    private void Update()
    {
        ...
        _text.text = $"Stamina: {Mathf.RoundToInt(_displayValue)}";
    }
